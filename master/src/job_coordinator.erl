...
-record(state, {jobinfo          :: jobinfo(),
                ...
                hosts      = gb_sets:empty()  :: disco_gbset(),
                % input | task_id() -> task_info().
                tasks      = gb_trees:empty() :: disco_gbtree(),
                % input_id() -> data_info().
                data_map   = gb_trees:empty() :: disco_gbtree(),
                % stage_name() -> stage_info().
                stage_info = gb_trees:empty() :: disco_gbtree()}).
-type state() :: #state{}.
...
% state.
-spec collect_runnable_deps(task_id(), disco_gbset(), state())
                           -> {[task_id()], state()}.
