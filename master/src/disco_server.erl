...
new_task(Task, Timeout) ->
    ...
    eflame:apply(normal_with_children, FileName, gen_server, call, [?MODULE,
            {new_task, Task}, Timeout]).
...
handle_call({new_task, Task}, _, S) ->
    ...
    Ret = eflame:apply(normal_with_children, FileName, ?MODULE, do_new_task, [Task, S]),
    {reply, Ret, S};
