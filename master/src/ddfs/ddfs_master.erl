...
-record(state, {tags      = gb_trees:empty()          :: disco_gbtree(),
                tag_cache = {false, gb_sets:empty()}  :: {boolean(), disco_gbset()},
                cache_refresher                       :: pid(),
                ...
                read_blacklist    = []                :: [node()],
                gc_blacklist      = []                :: [node()],
                safe_gc_blacklist = gb_sets:empty()   :: disco_gbset(),
...
-spec safe_gc_blacklist(disco_gbset()) -> ok.
...
-spec update_nodestats(disco_gbtree()) -> ok.
update_nodestats(NewNodes) ->
    gen_server:cast(?MODULE, {update_nodestats, NewNodes}).

-spec update_tag_cache(disco_gbset()) -> ok.
...
-spec handle_cast({tag_notify, ddfs_tag:cast_msg(), tagname()}
                  | {gc_blacklist, [node()]}
                  | {safe_gc_blacklist, disco_gbset()}
                  | {update_gc_stats, gc_stats()}
                  | {update_tag_cache, disco_gbset()}
                  | refresh_tag_cache
                  | {update_nodes, nodes_update()}
                  | {update_nodestats, disco_gbtree()},
                  state()) -> gs_noreply().
...
-spec get_tag_pid(tagname(), disco_gbtree(), {boolean(), disco_gbset()}) ->
                         {pid(), disco_gbtree()}.
...
-spec do_update_nodestats(disco_gbtree(), state()) -> state().
