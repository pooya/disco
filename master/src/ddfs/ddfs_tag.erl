...
-record(state, {tag  :: tagname(),
                ...
                delayed   = false :: false | disco_gbtree(),
                replicas  = false :: false | [node()],
                locations = false :: false | [url()],
                url_cache = false :: false | disco_gbset()}).
-type state() :: #state{}.
...
-spec gc_update_urls(tagid(), [[url()]], disco_gbtree(), [node()], tagid())
                    -> [[url()]].
...
-spec do_delayed_update([[url()]], [term()], replyto(), disco_gbtree(), state())
                       -> state().
do_delayed_update(Urls, Opt, ReplyTo, Buffer, S) ->
