...
-spec handle_file(path(), path(), volume_name(), object_type(),
    erlang:timestamp(), term())
                 -> 'ok'.
handle_file("!trash" ++ _, _, _, _, _, _) ->
    ok;
% GC1) Remove leftover !partial. files
handle_file("!partial" ++ _ = File, Dir, _, _, Now, Paranoid) ->
...
handle_file(Obj, Dir, VolName, Type, _, _) ->
